...
import { ArrowRightIcon, CursorArrowIcon, DownloadIcon, EraserIcon, ImageIcon, Pencil1Icon, PersonIcon, TextIcon } from "@radix-ui/react-icons"
import { AssetRecordType, MediaHelpers, TLAsset, TLAssetId, TLShapeId, TLTextShapeProps, getHashForString, isGifAnimated, useEditor } from "@tldraw/tldraw"
import { useEffect, useRef, useState } from "react"

import AddAssetPanel from "./AddAssetPanel"
import CharacterPickerOverlap from "./CharacterPickerOverlap"

type TLDrawToolId = 'select' | 'draw' | 'eraser' | 'arrow' | 'character' | 'text' | 'save'
...
export default function BookUI({ }: BookUIProps) {
    ...
    useEffect(() => {
        ...
        const handleChange = (e: any) => {
            ...
            if (Object.keys(changes.added).some((key) => key.includes("shape:"))) {
                let shapeId = Object.keys(changes.added).find((key) => key.includes("shape:"))!
                let type = changes.added[shapeId].type;
                switch (type) {
                    case "character": {
                        editor.updateShape({ id: shapeId as TLShapeId, props: { type: selectedCharacter.toString() } } as any)
                        setCurrentTool('select')
                        break;
                    }
                }
    ...
    const addAsset = async (url: string) => {
        ...
        let file = await fetch(url).then((res) => res.blob()).then((blob) => new File([blob], 'file', { type: blob.type }))
        console.log(file)
    ...
    const tools: Tool[] = [
        ...
        },
        {
            ...
            className: 'relative',
            overlappedComponent: <CharacterPickerOverlap showCharacter={showCharacter} setShowCharacter={setShowCharacter} handleCharacterSelect={handleCharacterSelect} />
        },
        {
            id: 'text',
            name: 'Text',
        ...
        },
        {
            ...
            icon: <DownloadIcon />,
            onClick: () => handleSave(),
            overlappedComponent: <AddAssetPanel showAssetPanel={showAssetPanel} setShowAssetPanel={setShowAssetPanel} addAsset={addAsset} setIsTyping={setIsTyping} />
        }
    ]
